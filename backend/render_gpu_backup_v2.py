...
class GPURenderContext:
    ...
    def preload(self, dataset, center_points, margin_m, vectors=None, 
        ...
        # FIX: Use explicit allocation and set() to avoid cudaErrorAlreadyMapped
        # which can happen when cp.asarray tries to register existing numpy memory as pinned.
        try:
            ortho_gpu = cp.empty(normalized.shape, dtype=normalized.dtype)
            ortho_gpu.set(normalized)
            self.ortho_texture = cp.ascontiguousarray(ortho_gpu.transpose(2, 0, 1))
        except Exception as e:
            print(f"[GPU] Error allocating/copying ortho texture: {e}")
            raise e
